...
test('표시 순서: 배열은 append-only여도 신규 그룹이 카탈로그 앞에 온다', async () => {
  ...
  assert.equal(order[0], 'Nex', `첫 그룹이 ${order[0]}`); // 2026-09-14 Nex-N2.5 추가 — 기존 그룹 상대 순서는 그대로
  assert.equal(order[1], 'Granite'); // 2026-09-05 Granite-4.2-30B 추가(#93)
  assert.equal(order[2], 'Spark'); // 2026-09-05 Spark-X2.5 추가
  assert.equal(order[3], 'Qwen 3.8');
  assert.equal(order[4], 'Laguna');
