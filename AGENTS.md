...
- **Precomputed data (CC0)**: `census/census-v1.{csv,json}` — 10,530 verdicts, model × device × quant. Also on [HF Datasets](https://huggingface.co/datasets/click6067/fitllm-fit-census).
...
2. **Engine math changes require conformance vectors to pass**: `node vectors/run.mjs` (33 byte-exact anchors). A port in any language is conformant iff all vectors pass.
