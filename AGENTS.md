...
**Purpose**: fitllm-engine answers "will this LLM fit this hardware?" with architecture-exact memory math (MLA / sliding-window / hybrid-linear / MoE — naive formulas are wrong by up to 17.8×).
...
2. **Engine math changes require conformance vectors to pass**: `node vectors/run.mjs` (14 byte-exact anchors). A port in any language is conformant iff all vectors pass.
